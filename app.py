...
app.config['SQLALCHEMY_DATABASE_URI'] = 'mariadb+mariadbconnector://root:root@localhost:3333/urbangym'
        ...
        if user:
            if user.is_confirmed:
                if user.is_admin:
                    ...
                    # Subquery to get the top category per day
                    subquery = (
                        db.session.query(
                            ProgramDetails.day,
                            Category.icone_Path,
                            Category.icone_Path_female,
                            func.count(Exercices.code).label('exercise_count')
                        )
                        .join(Exercices, ProgramDetails.exercice_code == Exercices.code)
                        .join(Category, Exercices.category_id == Category.Id)
                        .filter(ProgramDetails.program_id == user.program_id, ProgramDetails.type == 'exercise')
                        .group_by(ProgramDetails.day, Category.Id, Category.icone_Path, Category.icone_Path_female)
                        .order_by(ProgramDetails.day, desc(func.count(Exercices.code)))  # Order by count descending
                        .subquery()
                    )

                    # Get only the top category per day
                    days_with_top_category = (
                        db.session.query(
                            subquery.c.day,
                            subquery.c.icone_Path,
                            subquery.c.icone_Path_female
                        )
                        .group_by(subquery.c.day)  # Select only the first category per day
                        .all()
                    )

                    print(days_with_top_category)
                    return render_template('index.html', user=user, days=days_with_top_category)
            else:
